...
vital_signals_file = (
    "env_vital_signals.txt"  # the program concatenates data_folder + vsignal_file
)
cluster_folder = (
    "./clusters"  # Directory containing cluster and sequence of visit files
)
...
cluster_files = [
    ...
    if file.startswith("cluster") and file.endswith(".txt")
]
...
for r in range(R):
    for c in range(C):
        pygame.draw.rect(
            screen, (230, 230, 230), (c * CELLW, r * CELLH, CELLW, CELLH), 1
        )

print(f"Stats printed by plot_clusters_and_seq.py\n")
print(
    f"Colors of the victims follow the original env_vital_signals.txt and not the predicted value"
)
...
pygame.draw.line(
    screen,
    (255, 0, 0),
    (mid_c * CELLW, gen_min_r * CELLH),
    (mid_c * CELLW, gen_max_r * CELLH),
    2,
)
pygame.draw.line(
    screen,
    (255, 0, 0),
    (gen_min_c * CELLW, mid_r * CELLH),
    (gen_max_c * CELLW, mid_r * CELLH),
    2,
)
pygame.draw.rect(
    screen,
    ...
    (
        gen_min_c * CELLW,
        gen_min_r * CELLH,
        (gen_max_c - gen_min_c + 1) * CELLW,
        (gen_max_r - gen_min_r + 1) * CELLH,
    ),
    2,
)
...
for c, r in vict_coords:
    ...
    pygame.draw.circle(
        screen,
        color,
        (c * CELLW + CELLW / 2, r * CELLH + CELLH / 2),
        0.4 * min(CELLW, CELLH),
    )
    if r < R / 2:
        if c < C / 2:
